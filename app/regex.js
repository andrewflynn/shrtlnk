...
regex_list.push(/^https?:\/\/(?:www\.)?amazon\.com\/[\w\/-]*(B\w{9}).*$/.source);
short_list.push('https://amzn.com/$1');

// http://www.theonion.com/r/53187
regex_list.push(/^https?:\/\/(?:www\.)?theonion\.com\/.*?(\d+)$/.source);
...
regex_list.push(/^https?:\/\/(?:www\.)?stackoverflow\.com\/.*?(?:\d+)\/.*?(?:\d+)\#(\d+).*$/.source);
short_list.push('http://stackoverflow.com/a/$1');

// StackOverflow (question)
// http://stackoverflow.com/q/5718624
regex_list.push(/^https?:\/\/(?:www\.)?stackoverflow\.com\/.*?(\d+).*$/.source);
...
// NOTE: Ignores the final tag which is the user (who is logged in) tag
//       that is used by SO, but we don't care about
regex_list.push(/^https?:\/\/(?:www\.)?(.*?stackexchange)\.com\/.*?(?:\d+)\/.*?(?:\d+)\#(\d+).*$/.source);
short_list.push('http://$1.com/a/$2');

// StackOverflow (question)
// http://stackoverflow.com/q/164194
regex_list.push(/^https?:\/\/(?:www\.)?(.*?stackexchange)\.com\/.*?(\d+).*$/.source);
...
regex_list.push(/^https?:\/\/(?:www\.)?google\.com\/(?:(?:search)|(?:webhp))?.*q\=([^&]+).*\&tbm\=([^&]+).*$/.source);
...
// NOTE: Search type (eg image, news, video, etc) need to come first, otherwise
//       generic would match first
regex_list.push(/^https?:\/\/(?:www\.)?google\.com\/(?:(?:search)|(?:webhp))?.*tbm\=([^&]+).*\&q\=([^&]+).*$/.source);
...
regex_list.push(/^https?:\/\/(?:www\.)?google\.com\/(?:(?:search)|(?:webhp))?.*q\=([^&]+).*$/.source);
short_list.push('https://www.google.com/search?q=$1');

// shrtlnk
// http://bit.ly/shrt_lnk
regex_list.push(/^https?:\/\/(?:www\.)?chrome\.google\.com\/webstore\/detail\/shrtlnk\/nccahogoimgbhghcjmghidnnngigcagi.*$/.source);
short_list.push('http://bit.ly/shrt_lnk');

// Instagram
// http://instagr.am/
regex_list.push(/^https?:\/\/(?:www\.)?instagram\.com\/([^\?]+).*$/.source);
short_list.push('http://instagr.am/$1');

function shrtn(str) {
  for (var i = 0; i < regex_list.length; i++) {
    var regex = new RegExp(regex_list[i]);
